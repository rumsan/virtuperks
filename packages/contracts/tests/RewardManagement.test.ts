...
describe('RewardManagement Contract', function() {
  ...
  describe('Deployment', function() {
    it('should deploy successfully and set initial state', async function() {
      const { rewardManagement, appRegistry, APP_ID, user2 , participant1, admin1} = await fixture();
     ...
     console.log(ownerRole, 'from the test file owner role');
      ...
      expect(await appRegistry.hasRole(APP_ID, ownerRole, admin1.address)).to.be.true;
  ...
  describe('Task Creation and Management', function() {
    it('should create task with valid parameters', async function() {
      const { rewardManagement, user2, rewardToken, appRegistry, APP_ID , admin1} = await fixture();
      ...
      await expect(rewardManagement.connect(admin1).createTask(taskId, task, []))
        .to.emit(rewardManagement, 'TaskCreated')
        .withArgs(taskId, admin1.address);
