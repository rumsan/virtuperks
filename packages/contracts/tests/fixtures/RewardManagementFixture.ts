...
export async function deployRewardManagementFixture(): Promise<RewardManagementFixture> {
  ...
  const App = await appRegistry.createApp(
    APP_ID,
    TEST_APP_NAME,
    admin1!.address,
    false,
  ...
  const RewardManagementFactory = await ethers.getContractFactory(
  ...
  await appRegistry
    .connect(admin1)
    .grantRoleAdmin(APP_ID, DEFAULT_ADMIN_ROLE, factory.target);

  // Verify factory has admin role

  const factoryHasAdminRole = await appRegistry.hasRole(
    APP_ID,
    DEFAULT_ADMIN_ROLE,
    factory.target,
  );
  const adminRole = await appRegistry.hasRole(
    APP_ID,
    DEFAULT_ADMIN_ROLE,
    admin1.address,
  );
  ...
  console.log(
    `Setting up ${entityOwnerAddresses.length} entity owners:`,
  ...
  const entity = {
    name: 'Test Entity',
    url: 'https://testentity.example.com',
  ...
  const event = receipt?.logs.find(
    (log: any) =>
      log.fragment && log.fragment.name === 'RewardManagementCreated',
  );
  if (!event) throw new Error('RewardManagement creation event not found');
  const [rewardManagementAddress, aclAddress, name, eventEntityOwners] =
    event.args;
  ...
  //call getEntityOwners to verify owners
  const retrievedOwners = await factory
    .connect(admin1)
  ...
  await appRegistry
    .connect(admin1)
    .grantRoleAdmin(APP_ID, participantRole, participant1.address);
  await appRegistry
    .connect(admin1)
    .grantRoleAdmin(APP_ID, MINTER, user2.address);

  // Log role assignments for verification

  console.log(
    await appRegistry.getRoleAdmins(APP_ID, participantRole),
    'participant role',
  );
